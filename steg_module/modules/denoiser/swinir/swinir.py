...
def load_swinir(config):
    cfg = OmegaConf.load(config)
    swinir = instantiate_from_config(cfg.swinir)
    sd = torch.load(cfg.swinir.path, map_location="cpu")
    if "state_dict" in sd:
        sd = sd["state_dict"]
    sd = {
    ...
    print(f"load SwinIR from {cfg.swinir.path}")
    
    return swinir


def swin_noise(model, img, mag=True):
    scale = 512 // img.size(-1)
    
    if mag:
        img = 0.5 * (img + 1)
        
    img = model(F.interpolate(img, scale_factor = scale))
    img = F.interpolate(img, scale_factor = 1/scale)
