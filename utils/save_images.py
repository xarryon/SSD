...
def save_images_2(saved_all, epoch, folder, resize_to=None):
    original_images, watermarked_images, reconstructed_images, noised_images = saved_all

    images = original_images[:original_images.shape[0], :, :, :].cpu()
    watermarked_images = watermarked_images[:watermarked_images.shape[0], :, :, :].cpu()
    reconstructed_images = reconstructed_images[:reconstructed_images.shape[0], :, :, :].cpu()
    noised_images = noised_images[:noised_images.shape[0], :, :, :].cpu()
    ...
    diff_w2co = _normalize(torch.abs(images - watermarked_images))
    diff_w2no = _normalize(torch.abs(noised_images - watermarked_images))
    diff_w2re = _normalize(torch.abs(reconstructed_images - watermarked_images))
    ...
    if resize_to is not None:
        ...
        diff_w2co = F.interpolate(diff_w2co, size=resize_to)
        diff_w2no = F.interpolate(diff_w2no, size=resize_to)
        diff_w2re = F.interpolate(diff_w2re, size=resize_to)
    ...
    stacked_images = torch.cat(
        [images.unsqueeze(0), watermarked_images.unsqueeze(0), noised_images.unsqueeze(0), reconstructed_images.unsqueeze(0), diff_w2co.unsqueeze(0),
         diff_w2no.unsqueeze(0), diff_w2re.unsqueeze(0)], dim=0)
    shape = stacked_images.shape
    stacked_images = stacked_images.permute(0, 3, 1, 4, 2).reshape(shape[3] * shape[0], shape[4] * shape[1], shape[2])
    # Add 0.5 after unnormalizing to [0, 255] to round to nearest integer
    stacked_images = stacked_images.mul(255).add_(0.5).clamp_(0, 255).to('cpu', torch.uint8).numpy()
...
def get_random_images(images, encoded_images_1, noised_images):
    selected_id = np.random.randint(1, images.shape[0]) if images.shape[0] > 1 else 1
    image = images.cpu()[selected_id - 1:selected_id, :, :, :]
    encoded_images_1 = encoded_images_1.cpu()[selected_id - 1:selected_id, :, :, :]
    noised_image = noised_images.cpu()[selected_id - 1:selected_id, :, :, :]
    return [image, encoded_images_1, noised_image]


def concatenate_images(saved_all, images, encoded_images_1, noised_images):
    saved = get_random_images(images, encoded_images_1, noised_images)
...
def get_random_images_2(images, encoded_images_1, encoded_images_2, noised_images):
    selected_id = np.random.randint(1, images.shape[0]) if images.shape[0] > 1 else 1
    image = images.cpu()[selected_id - 1:selected_id, :, :, :]
    encoded_images_1 = encoded_images_1.cpu()[selected_id - 1:selected_id, :, :, :]
    encoded_images_2 = encoded_images_2.cpu()[selected_id - 1:selected_id, :, :, :]
    noised_image = noised_images.cpu()[selected_id - 1:selected_id, :, :, :]
    return [image, encoded_images_1, encoded_images_2, noised_image]


def concatenate_images_2(saved_all, images, encoded_images_1, encoded_images_2, noised_images):
    saved = get_random_images_2(images, encoded_images_1, encoded_images_2, noised_images)
    if saved_all[2].shape[2] != saved[2].shape[2]:
        return saved_all
    saved_all[0] = torch.cat((saved_all[0], saved[0]), 0)
    saved_all[1] = torch.cat((saved_all[1], saved[1]), 0)
